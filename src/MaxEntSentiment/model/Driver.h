...
public:
	...
	inline void initial() {
		...
		setUpdateParameters(_hyperparams._regParameter, _hyperparams._adaAlpha, _hyperparams._adaEps);
	}
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			//}
			cost += _modelparams.loss.loss(&_pcg->_sparsenode, example.m_label, _eval, example_num);
	...
	inline void predict(const Feature& feature, int& result) {
		...
		//}
		_modelparams.loss.predict(&_pcg->_sparsenode, result);
	}
	...
	inline dtype cost(const Example& example){
		...
		//}
		cost += _modelparams.loss.cost(&_pcg->_sparsenode, example.m_label, 1);
